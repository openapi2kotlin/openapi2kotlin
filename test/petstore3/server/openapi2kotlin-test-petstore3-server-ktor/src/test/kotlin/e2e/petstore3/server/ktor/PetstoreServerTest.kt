...
class PetstoreServerTest {
    @Test
    fun `pet findByStatus route returns seeded pet`() = testApplication {
        application {
            ...
            routing {
                petRoutes(
                    object : PetApi {
                        ...
                        override suspend fun deletePet(apiKey: String?, petId: Long) = Unit
                        ...
                        override suspend fun createPet(petId: Long, name: String?, status: String?): Pet =
                        ...
                        override suspend fun createUploadImage(
                            ...
                            body: ByteArray?,
                        ) = e2e.petstore3.server.ktor.generated.model.ApiResponse(code = 200, message = additionalMetadata)
                    }
                )
            }
        }
    ...
    @Test
    fun `store inventory route returns json inventory`() = testApplication {
        application {
            install(ContentNegotiation) {
                json(Json { ignoreUnknownKeys = true })
            }
